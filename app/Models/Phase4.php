...
class Phase4 extends Model
{
    ...
    public static function getPhase4Json($job_uuid = '')
    {
        ...
        return DataTables::of($application)
            ...
            ->addColumn('nilai', function ($row) {
                if (is_null($row->phase4)) {
                    return '-';
                }
                if (!empty($row->phase4->nilai_akhir)) {
                    $nilai = $row->phase4->nilai_akhir;
                } else {
                    $nilai = 'Belum di Nilai';
                }
                $html = '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#bd-' . $row->phase4->id . '">' . $nilai . '</button>
                <div id="bd-' . $row->phase4->id . '" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                  <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        ' . view('sdm.nilai.n_phase4', compact('row'))->render() . '
                        </div>
            ...
            ->addColumn('lolos', function ($row) {
                ...
                if ($row->phase4->status == 1) {
                    $ya = 'checked';
                    $tidak = '';
                } elseif ($row->phase4->status == 2) {
                    $ya = '';
                    $tidak = 'checked';
                } else {
                    $ya = '';
    ...
    public static function statusPhase4($request, $uuid)
    {
        $phase = Phase4::where('application_id', getAppId($uuid)->id)->first();

        $app = Application::findOrFail($phase->application_id);

        $app->update([
            'phase4s' => $phase->nilai_akhir
        ]);
