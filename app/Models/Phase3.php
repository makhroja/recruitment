...
class Phase3 extends Model
{
    ...
    public static function getPhase3Json($job_uuid = '')
    {
        ...
        return DataTables::of($application)
            ...
            ->addColumn('nilai', function ($row) {
                if (is_null($row->phase3)) {
                    return '-';
                }
                if (!empty($row->phase3->nilai_akhir)) {
                    $nilai = $row->phase3->nilai_akhir;
                } else {
                    $nilai = 'Belum di Nilai';
                }
                $html = '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#bd-' . $row->phase3->id . '">' . $nilai . '</button>
                <div id="bd-' . $row->phase3->id . '" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                  <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        ' . view('sdm.nilai.n_phase3', compact('row'))->render() . '
                        </div>
            ...
            ->addColumn('lolos', function ($row) {
                ...
                if ($row->phase3->status == 1) {
                    $ya = 'checked';
                    $tidak = '';
                } elseif ($row->phase3->status == 2) {
                    $ya = '';
                    $tidak = 'checked';
                } else {
                    $ya = '';
    ...
    public static function statusPhase3($request, $uuid)
    {
        $phase = Phase3::where('application_id', getAppId($uuid)->id)->first();

        $app = Application::findOrFail($phase->application_id);

        $app->update([
            'phase3s' => $phase->nilai_akhir
        ]);
