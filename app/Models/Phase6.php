...
class Phase6 extends Model
{
    ...
    public static function getPhase6Json($job_uuid = '')
    {
        ...
        return DataTables::of($application)
            ...
            ->addColumn('nilai', function ($row) {
                if (is_null($row->phase6)) {
                    return '-';
                }
                if (!empty($row->phase6->nilai_akhir)) {
                    $nilai = $row->phase6->nilai_akhir;
                } else {
                    $nilai = 'Belum di Nilai';
                }
                $html = '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#bd-' . $row->phase6->id . '">' . $nilai . '</button>
                <div id="bd-' . $row->phase6->id . '" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                  <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        ' . view('sdm.nilai.n_phase6', compact('row'))->render() . '
                        </div>
            ...
            ->addColumn('lolos', function ($row) {
                ...
                if ($row->phase6->status == 1) {
                    $ya = 'checked';
                    $tidak = '';
                } elseif ($row->phase6->status == 2) {
                    $ya = '';
                    $tidak = 'checked';
                } else {
                    $ya = '';
    ...
    public static function statusPhase6($request, $uuid)
    {
        $phase = Phase6::where('application_id', getAppId($uuid)->id)->first();

        $app = Application::findOrFail($phase->application_id);

        $app->update([
            'phase6s' => $phase->nilai_akhir
        ]);
