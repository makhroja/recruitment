...
class Phase8 extends Model
{
    ...
    public static function getPhase8Json($job_uuid = '')
    {
        ...
        return DataTables::of($application)
            ...
            ->addColumn('nilai', function ($row) {
                if (is_null($row->phase8)) {
                    return '-';
                }
                if (!empty($row->phase8->nilai_akhir)) {
                    $nilai = $row->phase8->nilai_akhir;
                } else {
                    $nilai = 'Belum di Nilai';
                }
                $html = '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#bd-' . $row->phase8->id . '">' . $nilai . '</button>
                <div id="bd-' . $row->phase8->id . '" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                  <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        ' . view('sdm.nilai.n_phase8', compact('row'))->render() . '
                        </div>
            ...
            ->addColumn('lolos', function ($row) {
                ...
                if ($row->phase8->status == 1) {
                    $ya = 'checked';
                    $tidak = '';
                } elseif ($row->phase8->status == 2) {
                    $ya = '';
                    $tidak = 'checked';
                } else {
                    $ya = '';
    ...
    public static function statusPhase8($request, $uuid)
    {
        $phase = Phase8::where('application_id', getAppId($uuid)->id)->first();

        $app = Application::findOrFail($phase->application_id);

        $app->update([
            'phase8s' => $phase->nilai_akhir
        ]);
