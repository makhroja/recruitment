...
<div class="form-group {{ $errors->has('job_id') ? 'has-error' : '' }}">
    ...
    <div class="col-md-12">
        <select style="width: 85%" class="form-control text-dark select2 @error('job_id') is-invalid @enderror"
            id="job_id" name="job_id">
            <option value="{{ $job->id }}" style="display: none;" {{ old('job_id') }} selected>{{ $job->judul }}
            </option>
        </select>
...
<div class="form-group {{ $errors->has('tahap') ? 'has-error' : '' }}">
    ...
    <div class="col-md-10">
        <select style="width: 35%" class="form-control text-dark select2 @error('tahap') is-invalid @enderror"
            id="tahap" name="tahap">
            @for ($i = 1; $i < 15; $i++)
                <option value="{{ $i }}" style="" {{ old('tahap') }}>
                    {{ 'Tahap ' . $i }}
                </option>
            @endfor
        </select>
...
<div class="form-group {{ $errors->has('nama_tahap') ? 'has-error' : '' }}">
    ...
    <div class="col-md-10">
        <input class="form-control  @error('nama_tahap') is-invalid @enderror" name="nama_tahap" type="text"
            id="nama_tahap" value="{{ old('nama_tahap') }}" minlength="1" placeholder="Isikan nama tahap disini...">
...
<div class="form-group {{ $errors->has('tgl_mulai') ? 'has-error' : '' }}">
    ...
    <div class="col-md-7">
        <input class="form-control  @error('tgl_mulai') is-invalid @enderror" name="tgl_mulai" type="date"
            id="tgl_mulai" value="{{ old('tgl_mulai') }}" placeholder="Isikan tgl mulai disini...">
